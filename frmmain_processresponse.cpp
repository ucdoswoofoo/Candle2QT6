...
void frmMain::ProcessGRBL1_1()
{
    while(SerialIf_CanReadLine())
    {
        ...
        if(data[0] == '<')
        {
            ...
            QRegularExpressionMatch  match1 = stx.match(data);
            if( match1.hasMatch() )
            //if (stx.indexIn(data) != -1)
            {
                status = m_status.indexOf(match1.captured(1));
            ...
            QRegularExpressionMatch match4 = wpx.match(data);
            //if(wpx.indexIn(data) != -1)
            if( match4.hasMatch() )
            {
                workOffset = QVector3D(match4.captured(1).toDouble(), match4.captured(2).toDouble(), match4.captured(3).toDouble());

                // Store offsets for rotary axis
                if(m_settings->UseRotaryAxis())
                {
                    workOffsetAB[0] = match4.captured(4).toDouble();
                    workOffsetAB[1] = match4.captured(5).toDouble();
                }
            }
            ...
            QRegularExpressionMatch matchov = ov.match(data);
            //if(ov.indexIn(data) != -1)
            if( matchov.hasMatch() )
            {
                UpdateOverride(ui->slbFeedOverride, matchov.captured(1).toInt(), 0x91);
                UpdateOverride(ui->slbSpindleOverride, matchov.captured(3).toInt(), 0x9a);

                int rapid = matchov.captured(2).toInt();
                ...
                QRegularExpressionMatch matchpn = pn.match(data);
                //if(pn.indexIn(data) != -1)
                if( matchpn.hasMatch() )
                {
                    pinState.append(QString(tr("PS: %1")).arg(matchpn.captured(1)));
                }

                // Process spindle state
                static QRegularExpression  as("A:([^,^>^|]+)");
                QRegularExpressionMatch matchas = as.match(data);
                //if(as.indexIn(data) != -1)
                if( matchas.hasMatch() )
                {
                    QString state = matchas.captured(1);
                    ...
                    pinState.append(QString(tr("AS: %1")).arg(matchas.captured(1)));
                }
                else
                {
                    m_timerToolAnimation.stop();
            ...
            QRegularExpressionMatch mastchfs = fs.match(data);
            //if(fs.indexIn(data) != -1)
            if( mastchfs.hasMatch() )
            {
                ui->glwVisualizer->setSpeedState((QString(tr("F/S: %1 / %2")).arg(mastchfs.captured(1)).arg(mastchfs.captured(2))));
            }
            ...
            if(mCommandsSent.size() > 0 && !DataIsFloating(data) && !(val.command != "[CTRL+X]" && DataIsReset(data)))
            {
                ...
                if((mCommandsSent.front().command != "[CTRL+X]" && DataIsEnd(data)) || (mCommandsSent.front().command == "[CTRL+X]" && DataIsReset(data)))
                {
                    ...
                    if(response.contains(QString("[GC")))
                    {
                        ...
                        QRegularExpressionMatch matchrx = rx.match(response);
                        //if(rx.indexIn(response) != -1)
                        if ( matchrx.hasMatch() )
                        {
                            double speed = toMetric(matchrx.captured(1).toDouble()); //RPM in imperial?
                    ...
                    if(ca.command.toUpper() == "$G" && ca.tableIndex == -3)
                    {
                        ...
                        // Spindle speed
                        QRegularExpression  rx(".*S([\\d\\.]+)");
                        QRegularExpressionMatch matchrx2 = rx.match(response);
                        //if(rx.indexIn(response) != -1)
                        if ( matchrx2.hasMatch() )
                        {
                            double speed = toMetric(matchrx2.captured(1).toDouble()); //RPM in imperial?
                            ui->slbSpindle->setCurrentValue(speed);
                        }
                    ...
                    if(ca.command == "$#" && ca.tableIndex == -2)
                    {
                        ...
                        QRegularExpressionMatch matchrx3 = rx.match(response);
                        //if(rx.indexIn(response) != -1)
                        if ( matchrx3.hasMatch() )
                        {
                            if(m_settingZeroX)
                            {
                                m_settingZeroX = false;
                                m_storedX = toMetric(matchrx3.captured(1).toDouble());
                            }
                            else if(m_settingZeroXY)
                            {
                                m_settingZeroXY = false;
                                m_storedY = toMetric(matchrx3.captured(2).toDouble());
                            }
                            else if(m_settingZeroZ)
                            {
                                m_settingZeroZ = false;
                                m_storedZ = toMetric(matchrx3.captured(3).toDouble());
                            }
                    ...
                    if(ca.command.contains("G38.2") && m_heightMapMode && ca.tableIndex > -1)
                    {
                        ...
                        QRegularExpressionMatch  matchrx4 = rx.match(response);
                        double z = qQNaN();
                        //if(rx.indexIn(response) != -1)
                        if ( matchrx4.hasMatch() )
                        {
                            qDebug() << "probing coordinates:" << matchrx4.captured(1) << matchrx4.captured(2) << matchrx4.captured(3);
                            z = toMetric(matchrx4.captured(3).toDouble());
                        }
...
{
    {
        ...
        if(data[0] == '<')
        {
            ...
            QRegularExpressionMatch  matchmpx = mpx.match(data);
            //if(mpx.indexIn(data) != -1)
            if ( matchmpx.hasMatch() )
            {
                ui->txtMPosX->setText(matchmpx.captured(1));
                ui->txtMPosY->setText(matchmpx.captured(2));
                ui->txtMPosZ->setText(matchmpx.captured(3));
            }
            if(m_settings->UseRotaryAxis())
            {
                // Set A & B
                //qDebug() << "A: " << mpx.cap(4);
                ui->txtMPosA->setText(matchmpx.captured(4));
                ui->txtMPosB->setText(matchmpx.captured(5));
            }

            // Status
            static QRegularExpression  stx("<([^,^>^|]*)");
            QRegularExpressionMatch  matchstx = stx.match(data);
            //if (stx.indexIn(data) != -1)
            if ( matchstx.hasMatch() )
            {
                status = m_status.indexOf(matchstx.captured(1));
            ...
            QRegularExpressionMatch  matchwpx = wpx.match(data);
            //if(wpx.indexIn(data) != -1)
            if ( matchwpx.hasMatch() )
            {
                workOffset = QVector3D(matchwpx.captured(1).toDouble(), matchwpx.captured(2).toDouble(), matchwpx.captured(3).toDouble());

                // Store offsets for rotary axis
                if(m_settings->UseRotaryAxis())
                {
                    workOffsetAB[0] = matchwpx.captured(4).toDouble();
                    workOffsetAB[1] = matchwpx.captured(5).toDouble();
                }
            }
            ...
            if ( matchov.hasMatch() )
            {
                ...
                // Process spindle state
                static QRegularExpression  as; //("A:([^,^>^|]+)");
                QRegularExpressionMatch match2 = as.match("A:([^,^>^|]+)");
                //if(as.indexIn(data) != -1)
                if( match2.hasMatch() )
                {
                    QString state = match2.captured(1);
                    m_spindleCW = state.contains("S");
                    ...
                    pinState.append(QString(tr("AS: %1")).arg(match2.captured(1)));
                }
                else
                {
                    m_timerToolAnimation.stop();
            ...
            // Get feed/spindle values
            static QRegularExpression  fs; //("FS:([^,]*),([^,^|^>]*)");
            QRegularExpressionMatch match3 = fs.match("FS:([^,]*),([^,^|^>]*)");
            //if(fs.indexIn(data) != -1)
            if( match3.hasMatch() )
            {
                ui->glwVisualizer->setSpeedState((QString(tr("F/S: %1 / %2")).arg(match3.captured(1)).arg(match3.captured(2))));
            }
                ...
                if((mCommandsSent.front().command != "[CTRL+X]" && DataIsEnd(data)) || (mCommandsSent.front().command == "[CTRL+X]" && DataIsReset(data)))
                {
                    ...
                    if(ca.command == "$#" && ca.tableIndex == -2)
                    {
                        ...
                        if ( matchrx.hasMatch() )
                        {
                            ...
                            {
                                m_settingZeroX = false;
                                m_storedX = toMetric(matchrx.captured(1).toDouble());
                            }
                            else if(m_settingZeroXY)
                            {
                                m_settingZeroXY = false;
                                m_storedY = toMetric(matchrx.captured(2).toDouble());
                            }
                            else if(m_settingZeroZ)
                            {
                                m_settingZeroZ = false;
                                m_storedZ = toMetric(matchrx.captured(3).toDouble());
                            }
                    ...
                    if(ca.command.contains("G38.2") && m_heightMapMode && ca.tableIndex > -1)
                    {
                        ...
                        if( matchrx.hasMatch() )
                        {
                            qDebug() << "probing coordinates:" << matchrx.captured(1) << matchrx.captured(2) << matchrx.captured(3);
                            z = toMetric(matchrx.captured(3).toDouble());
                        }
