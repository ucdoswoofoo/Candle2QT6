    ...
    ui(new Ui::frmMain)
{
    ...
    //QRegularExpression qr = QRegularExpression("cmdUser\\d");
    //this->centralWidget()->findChildren()
    // foreach (StyledToolButton* button, this->centralWidget()->findChildren<StyledToolButton*>( QRegularExpression("cmdUser\\d")) )
    // {
    //     connect(button, SIGNAL(clicked(bool)), this, SLOT(onCmdUserClicked(bool)));
    // }
    StyledToolButton* button0 = ui->cmdUser0;
    connect(button0, SIGNAL(clicked(bool)), this, SLOT(onCmdUserClicked(bool)));
    StyledToolButton* button1 = ui->cmdUser1;
    connect(button1, SIGNAL(clicked(bool)), this, SLOT(onCmdUserClicked(bool)));
    StyledToolButton* button2 = ui->cmdUser2;
    connect(button2, SIGNAL(clicked(bool)), this, SLOT(onCmdUserClicked(bool)));
    StyledToolButton* button3 = ui->cmdUser3;
    connect(button3, SIGNAL(clicked(bool)), this, SLOT(onCmdUserClicked(bool)));
    ...
    // Prepare jog buttons
    StyledToolButton* button0a = ui->cmdUser0;
    connect(button0a, SIGNAL(clicked(bool)), this, SLOT(onCmdJogFeedClicked()));
    StyledToolButton* button1a = ui->cmdUser1;
    connect(button1a, SIGNAL(clicked(bool)), this, SLOT(onCmdJogFeedClicked()));
    StyledToolButton* button2a = ui->cmdUser2;
    connect(button2a, SIGNAL(clicked(bool)), this, SLOT(onCmdJogFeedClicked()));
    StyledToolButton* button3a = ui->cmdUser3;
    connect(button3a, SIGNAL(clicked(bool)), this, SLOT(onCmdJogFeedClicked()));
...
bool frmMain::DataIsReset(QString data)
{
    ...
    QRegularExpression  rc("^GRBL[^ ]*\\s\\d\\.\\d") ;
    QRegularExpressionMatch match = rc.match(data.toUpper());

    return match.hasMatch();
    //return QRegularExpression("^GRBL[^ ]*\\s\\d\\.\\d").indexIn(data.toUpper()) != -1;
}
...
{
    return ui->grpConsole->height() - ui->grpConsole->contentsRect().height() + ui->spacerConsole->geometry().height() + ui->grpConsole->layout()->spacing() * 2 + ui->cboCommand->height();
}
